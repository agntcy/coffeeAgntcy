...
Search the AGNTCY directory for remote A2A agents, present candidates, and connect them to Claude Code as **skills** (slash commands).
...
> **Critical:** Generated skills do **not** reliably get `${CLAUDE_PLUGIN_ROOT}` substituted at the skill's own invoke time. Resolve the `a2a-send` binary to an **absolute path** while `/recruit` runs (where `${CLAUDE_PLUGIN_ROOT}` *does* expand), and bake that absolute path into the generated `SKILL.md`. The only token left for skill-invoke-time substitution is `$ARGUMENTS`, which Claude Code does substitute reliably.
...
A2A_SEND_PATH="${CLAUDE_PLUGIN_ROOT}/scripts/a2a-send/a2a-send"
sed \
  ...
  "${CLAUDE_PLUGIN_ROOT}/scripts/skill-template.md" \
...
- `${CLAUDE_PLUGIN_ROOT}` and `${A2A_SEND_PATH}` are intentionally expanded by the shell at recruit time — this is how the absolute path gets baked in.

Verify by `cat`ing the resulting file and confirming:
- `__A2A_SEND_PATH__` is now an absolute path like `/Users/…/plugin/scripts/a2a-send/a2a-send` (no `${CLAUDE_PLUGIN_ROOT}` remaining).
...
- **File write failure**: Check permissions on `.claude/skills/`
