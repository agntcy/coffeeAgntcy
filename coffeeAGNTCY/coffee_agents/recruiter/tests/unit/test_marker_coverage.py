...
REQUIRED_LLM_CLASSES: tuple[tuple[str, str], ...] = (
    ("tests.integration.test_agent_evaluator", "TestEvaluationIntegration"),
)

REQUIRED_LLM_MODULES: tuple[str, ...] = (
    "tests.integration.test_a2a",
    "tests.integration.test_caching",
)
...
def _module_has_llm_marker(module_name: str) -> bool:
    module = importlib.import_module(module_name)
    return any(mark.name == "llm" for mark in _iter_marks(module))
...
@pytest.mark.parametrize("module_name", REQUIRED_LLM_MODULES)
def test_llm_integration_modules_are_marked(module_name: str) -> None:
    assert _module_has_llm_marker(module_name), (
        f"{module_name} must set pytestmark = pytest.mark.llm"
    )
