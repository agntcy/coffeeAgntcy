...
To expose the farm servers over HTTP, include the `ENABLE_HTTP` environment variable in each command as shown below:

```sh
ENABLE_HTTP=true uv run python farms/brazil/farm_server.py
ENABLE_HTTP=true uv run python farms/colombia/farm_server.py 
ENABLE_HTTP=true uv run python farms/vietnam/farm_server.py 
```

Once these commands are executed, the agent cards will be accessible at the following URL: `http://example-farm-server/.well-known/agent.json`.
...
### Prerequisites
1. **Install ngrok CLI**: Download and install the ngrok CLI from [ngrok's official website](https://ngrok.com/download).
2. **Run the `start_ngrok.sh` Script**: This script creates a tunnel to your local machine on port 4444 and exposes it to the public internet. It also updates the `IDP_ISSUER_URL` in `docker-compose.yml` and the `issuer` field in `hydra.yml`.
...
3. **Run the Exchange Server**  
   Start the Exchange server locally. Ensure the `REGISER_ISSUER` environment variable is set to `true` if the `start_ngrok.sh` script was recently executed or the ngrok URL has changed:
   ```bash
   REGISER_ISSUER=true IDP_ISSUER_URL="https://<ngrok-url>" uv run python exchange/main.py
   ```
   If the ngrok URL has not changed, set `REGISER_ISSUER` to `false`.

### Notes
- Always set `REGISER_ISSUER=true` after running the `start_ngrok.sh` script or when the ngrok URL changes.
