...
class WorkflowMetadata:
    ...
    use_case: str
    scenario: str
    scenario_documentation_path: str
...
def _load_catalog() -> dict[str, WorkflowMetadata]:
    ...
    for idx, entry in enumerate(data):
        ...
        scenario_documentation_path = entry.get("scenario_documentation_path")
        if not (
            isinstance(name, str)
            and isinstance(pattern, str)
            and isinstance(use_case, str)
            and isinstance(scenario, str)
            and isinstance(scenario_documentation_path, str)
        ):
            logger.warning(
                "Skipping workflow at index %d: missing name/pattern/use_case/scenario/scenario_documentation_path, entry: %s",
                idx,
                entry,
            )
            continue
        ...
        catalog[name] = WorkflowMetadata(
            ...
            use_case=use_case,
            scenario=scenario,
            scenario_documentation_path=scenario_documentation_path,
        )
