...
"""Trace-scoped in-flight state for A2A event middleware.

A2A interceptor and consumer callbacks receive asymmetric context:
- The interceptor has ClientCallContext and can read tool/workflow data.
- The consumer callback does not receive ClientCallContext.

To bridge that gap, this module stores per-trace interaction state when
