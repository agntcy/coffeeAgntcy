...
"""Workflow catalog loading and tool-to-workflow resolution for A2A events.

Why this exists:
- Event middleware emits workflow-pattern/use-case metadata with A2A topology events.
- Interceptor logic only sees ``context.state["tool"]`` at runtime.

How mapping works:
- Tool call sites in supervisor modules use ``@register_workflow(...)``.
- ``make_tool_call_context(...)`` writes a normalized tool key into context.
- ``ToolWorkflowResolver`` maps that key to ``WorkflowIdentity`` metadata.
"""
...
class WorkflowIdentity:
	...
	Long-term: replace this local type with a shared schema type,
	for example WorkflowMetadata.
	"""
...
def register_workflow(workflow_name: str):
	"""Decorator for binding a tool call site to a catalog workflow name."""
...
def make_tool_call_context(tool_obj: Any, **extra: Any) -> ClientCallContext:
	"""Build ClientCallContext with the normalized tool key for resolver lookup."""
...
class ToolWorkflowResolver:
	"""Resolve the context tool key to workflow metadata for event emission."""
