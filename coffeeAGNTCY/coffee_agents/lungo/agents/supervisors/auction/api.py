...
def create_apps_router():
    ...
    async def resolve_app_by_slug(slug: str):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail="Unable to fetch apps from the identity service.",
            )
    ...
    async def get_policies_for_app(slug: str):
        ...
        if not identity_service.api_key:
            raise HTTPException(
                status_code=500, detail="Identity service API key is missing."
            )
        app = await resolve_app_by_slug(slug)
        try:
            policies_model = await identity_service.list_policies()
            filtered_policies = [
                p for p in policies_model.policies if p.assignedTo == app.id
            ]
            logger.info(
                f"Resolved slug '{slug}' to app '{app.id}', found {len(filtered_policies)} policies."
            )
            return {
                "app": {
                    "id": app.id,
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to fetch policies from the identity service.",
            )
        except Exception as e:
            logger.exception(f"Unexpected error: {e}")
            raise HTTPException(
                status_code=500,
                detail="An unexpected error occurred while fetching policies.",
            )
    ...
    async def get_badge_for_app(slug: str):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to fetch badge from the identity service.",
            )
        except Exception as e:
            logger.exception(f"Unexpected error: {e}")
            raise HTTPException(
                status_code=500,
                detail="An unexpected error occurred while fetching the badge.",
            )
