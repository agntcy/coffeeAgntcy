...
def test_supervisor_raises_when_llm_does_not_support_streaming(
    monkeypatch, real_workflow_catalog, import_module, expected_agent_name
):
    ...
    try:
        with patch(
            "litellm.get_model_info", return_value={"supports_native_streaming": False}
            ...
            try:
                ...
                assert type(e).__name__ == "StreamingNotSupportedError", (
                    f"Expected StreamingNotSupportedError, got {type(e)}"
                )
                ...
                model_in_call = (
                    call_kw.get("model")
                    if call_kw
                    else (call_args[0] if call_args else None)
                )
                assert model_in_call, (
                    f"get_model_info must be called with a model, got {model_in_call!r}"
                )
                return
