...
class IdentityServiceImpl(IdentityService):
    ...
    def get_all_apps(self) -> IdentityServiceApps:
        ...
        raise ValueError(
            f"Failed to fetch apps: {response.status_code}, {response.text}"
        )
    ...
    def get_badge_for_app(self, app_id: str) -> Badge:
        ...
        raise ValueError(
            f"Failed to fetch badge for app {app_id}: {response.status_code}, {response.text}"
        )
    ...
    def verify_badges(self, badge: Badge) -> Dict[str, Any]:
        ...
        raise ValueError(
            f"Failed to verify badge: {response.status_code}, {response.text}"
        )
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if attempt == MAX_RETRIES:
                    raise ValueError(
                        f"Failed to create badge after {MAX_RETRIES} attempts: {e}"
                    )
    ...
    async def list_policies(self) -> Policies:
        ...
        if response.status_code != 200:
            raise ValueError(
                f"Failed to fetch policies: {response.status_code}, {response.text}"
            )
